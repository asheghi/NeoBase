<template>
  <nav class="DashNavBar">
    <div class="hidden lg:flex">
      <NButton class="gap-2 px-0 mr-4" @click="$router.push({ name: 'projects' })">
        <IconLogo />
        NeoBase
      </NButton>
    </div>
    ...
    <NButton v-if="$route.name !== 'projects'" class="projects" @click="projectsDropped = !projectsDropped">
      {{ $route.params?.project }}
      <div v-if="!project.fetching" :class="{ block: projectsDropped, hidden: !projectsDropped }" class="drop-down">
        <NButton v-for="p in projects.data?.filter(
  (it) => it.name !== $route.params.project
)" :key="p._id" class="item" @click.prevent.stop="onProjectSelected(p.name)">
          {{ p.name }}
        </NButton>
      </div>
    </NButton>
    <NButton class="square" @click="toggleDarkMode">
      <component :is="isDarkMode ? IconLightMode : IconDarkMode" class="toggle" width="28" height="28" />
    </NButton>
    <NButton class="square">
      <IconNotifications />
    </NButton>
    <NButton class="square account">
      <IconAccount />
    ...
    <div class="nav-drawer" :class="{ expanded, 'hide-nav': !project }">
      ...
      <div v-if="project" class="links">
        <router-link v-for="(tab, index) in sidebarItems" :key="index" class="item" :to="tab.to"
    ...
    <transition name="fade">
      <div v-if="expanded" class="nav-drawer-shadow lg:hidden" @click="expanded = false"></div>
    </transition>
  </nav>
</template>
<script setup>
...
<style lang="scss">
.DashNavBar {
  @apply fixed inset-x-0 top-0 flex px-2 py-2 items-center justify-between bg-white border-b border-gray-300 dark:border-gray-500 z-10;
